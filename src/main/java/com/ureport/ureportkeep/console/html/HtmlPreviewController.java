...
public class HtmlPreviewController extends AbstractReportBasicController {
    ...
    public String preview(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (htmlReport == null) {
            ...
            Object si = buildParameters(request).get("si");
            int nums = 0;
            if (si != null) {
                nums = si.toString().length() - si.toString().replaceAll(",", "").length();
            }
            if (file.contains("salary") && (nums > Integer.parseInt(Objects.requireNonNull(env.getProperty("ifRunDataxNums"))) || si == null)) {
                model.addAttribute("isnull", "由于数据量太大，故不展示，请直接点击导出Excel下载全部");
            }
        ...
        if (file.equals(PREVIEW_KEY)) {
            ...
            if (StringUtils.isNotBlank(pageIndex) && !pageIndex.equals("0")) {
                ...
                if (file.contains("salary") && (nums > Integer.parseInt(Objects.requireNonNull(env.getProperty("ifRunDataxNums"))) || si == null)) {
                    ...
                    htmlReport = exportManager.exportHtml(file, req.getContextPath(), tmp_parameters);
                    //可以在这里获取表头
    ...
    public String getTitleName(String file, HashMap<String, Object> parameters) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            System.out.println(map.get(columns.get(i)).getFormatData().toString());
